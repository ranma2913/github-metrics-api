...
class FileWriterUtilForCodeQL {
  ...
  static def writeSimpleXlsxFile(Path outputFilePath, List<String> headerRowData, List<List<String>> rowData, String sheetName) {
    ...
    XSSFCell headerCodeQLMergeStatus = headerRow.createCell(3, CellType.STRING)
    headerCodeQLMergeStatus.setCellValue(headerRowData[3])
    ...
    for (List<String> csvRow in rowData) {
      ...
      XSSFCell codeQLMergeStatus = row.createCell(3, CellType.BOOLEAN)
      codeQLMergeStatus.setCellValue(csvRow[3])
